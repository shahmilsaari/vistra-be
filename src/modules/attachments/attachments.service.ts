...
export class AttachmentsService {
  ...
  async createMultiple(
    ...
    console.log(`🔄 Creating ${files.length} attachment(s) in transaction...`);
    ...
    const attachments = await this.prisma.$transaction(async (tx) => {
      ...
      const created = await tx.attachment.findMany({
        where: {
          storageKey: { in: records.map((record) => record.storageKey) },
          userId,
        },
        include: {
          user: true,
      ...
      // Preserve original order for predictable responses
    ...
    console.log(`✅ Transaction completed! ${attachments.length} attachment(s) created`);
    ...
    Promise.all(
      ...
      .catch((error) => {
        console.error('❌ Failed to create upload logs:', error.message);
      });
  ...
  async list(query: QueryAttachmentsDto): Promise<PaginatedAttachmentsDto> {
    ...
    // Build orderBy based on query parameters
  ...
  async update(
    ...
    this.logsService
      ...
      .catch((error) => {
        console.warn(`⚠️ Failed to record update log: ${error.message}`);
      });
  ...
  async deleteDirectory(folder: string, userId: number) {
      ...
      attachments.map(async (attachment) => {
        ...
        try {
          ...
          if (error.code !== 'ENOENT') {
            console.warn(`⚠️ Failed to remove file ${attachment.storageKey}: ${error.message}`);
          }
        }
      }),
    );
    ...
    try {
      ...
      console.warn(`⚠️ Failed to remove directory: ${error.message}`);
    }
  ...
  async deleteAttachment(id: number, userId: number) {
    ...
    try {
      ...
      if (error.code !== 'ENOENT') {
        console.warn(`⚠️ Failed to remove file from disk: ${error.message}`);
      }
    }
    ...
    this.logsService
      ...
      .catch((error) => {
        console.warn(`⚠️ Failed to record delete log: ${error.message}`);
      });
